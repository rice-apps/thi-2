...
interface EditTeacherProps {
  toggleEditTeacher: () => void;
}

export const EditTeacher: React.FC<EditTeacherProps> = ({ toggleEditTeacher }) => {
  ...
  return (
    ...
    >
      ...
      <View className="flex flex-row bg-[#105366] h-[64px] items-center justify-between px-6">
        <Text className="text-white text-xl font-semibold">Edit Teacher</Text>
        <FontAwesome6 name="xmark" size={22} color="white" onPress={toggleEditTeacher} />
      </View>
      ...
      <View className="flex m-8 justify-between gap-4">
        ...
        <View className="flex">
          ...
          <TextInput className="h-[48px] border rounded-lg mt-4 p-2.5 text-base color-gray-500" placeholder="Enter name" />
        </View>

        {/* Teacher Email */}
        <View>
          <Text className="font-bold text-xl">Teacher Email</Text>
          <TextInput className="h-[48px] border rounded-lg mt-4 p-2.5 text-base color-gray-500" placeholder="Enter email" />
      ...
      <View className="absolute bottom-0 flex flex-row border-t w-full h-[64px] items-center justify-end gap-4 pr-4">
        <Text className="text-base color-[#105366] font-semibold" onPress={toggleEditTeacher}>Cancel</Text>
        <TouchableOpacity className="shadow bg-[#105366] rounded-lg pt-2 pr-6 pl-6 pb-2" onPress={toggleEditTeacher}>
